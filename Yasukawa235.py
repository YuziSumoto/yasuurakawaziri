...
from MstService import *   # サービスマスタ
...
#      return
    ...
    path = os.path.join(os.path.dirname(__file__), 'Yasukawa235.html')
...
#------------------------------------------------------------------------------
  def DBSet(self):  # データ保存
    ...
    if Rec.CD != 0: # コード指定時は手入力無視
      RecService = MstService().GetRec(Rec.CD)
