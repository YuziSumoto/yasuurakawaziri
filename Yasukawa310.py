...
#-------------------------------------------------------------
  def get(self):
    ...
    if  self.request.get('Key') == "":
      ...
      Rec = DatSoudan().GetRec(self.request.get('Key'))
    ...
    path = os.path.join(os.path.dirname(__file__), 'Yasukawa310.html')
...
#-------------------------------------------------------------
  def post(self):
    ...
    for param in self.request.arguments():
      if "BtnEnd" in param:
        self.DBSet() # 画面保存 
        self.redirect("/Yasukawa300/?KanzyaID=" + str(Key)) #
      if "BtnKeika" in param:
        self.DBSet() # 画面保存 
        self.redirect("/Yasukawa320/?Key=" + str(Key)) #
...
#------------------------------------------------------------------------------
  def DBSet(self):  # データ保存
    ...
    if Key == "":
      Rec = DatSoudan() # 新規レコード
    else:
      Rec = DatSoudan().GetRec(Key) # 更新
